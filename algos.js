...
const solve = (n) => {
  if (
    n
      .toString()
      .split("")
      .reduce((a, b) => parseInt(a) + parseInt(b)) >= 10
  ) {
    n = n
      .toString()
      .split("")
...
const solve = (n) => {
  ...
  for (let i = 0; i < nString.length; i++) {
    variations.push(
      parseInt(nString.slice(0, i) + "3" + nString.slice(i + 1, nString.length))
    );
  }
...
const solve = (nums) => {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    if (next > current && (increasing === null || increasing === true)) {
      ...
      next < current &&
      (increasing === null || increasing === false)
    ) {
      increasing = false;
      continue;
...
const solve = (prices) => {
  ...
  const sorted = pricesDuplicate.sort(function (a, b) {
    return b - a;
  });
